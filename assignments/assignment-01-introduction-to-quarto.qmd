...
1. Use the `lm()` function to fit Model 1 from the slides. Use the `tidy()` function from the `{broom}` package to display the model's coefficient-level output. Display the syntax for this along with the coefficient-level output in your rendered document.
