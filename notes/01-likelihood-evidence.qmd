...
\mathbf{Model~2:~} \quad \mathrm{News~Knowledge}_i = &\beta_0 + \beta_1(\mathrm{Age}_i) + \beta_2(\mathrm{Education}_i) + \beta_3(\mathrm{Male}_i) + \beta_4(\mathrm{Engagement}_i) + \beta_5(\mathrm{Ideology}_i)+ \beta_6(\mathrm{Democrati}_) + \\ &\beta_7(\mathrm{Republican}_i) + \beta_8(\mathrm{Blue~State}_i) + \beta_9(\mathrm{News~Exposure}_i) + \epsilon_i \\[2ex]
\mathbf{Model~3:~} \quad \mathrm{News~Knowledge}_i = &\beta_0 + \beta_1(\mathrm{Age}_i) + \beta_2(\mathrm{Education}_i) + \beta_3(\mathrm{Male}_i) + \beta_4(\mathrm{Engagement}_i) + \beta_5(\mathrm{Ideology}_i)+ \beta_6(\mathrm{Democrati}_) + \\ &\beta_7(\mathrm{Republican}_i) + \beta_8(\mathrm{Blue~State}_i) + \beta_9(\mathrm{News~Exposure}_i) + \beta_{10}(\mathrm{News~Exposure}_i)(\mathrm{Education}_i) + \epsilon_i \\[2ex]
\end{split}
$$
...
lm.1 = lm(peer_rating ~ 1 + ft_students + pct_doc + gre + doc_acc + tot_pubs, data = usnwr_complete)
